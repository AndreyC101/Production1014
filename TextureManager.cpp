...
void TextureManager::modifiedDraw(std::string id, int x, int y, int SpriteMax, int xOffest, SDL_Renderer* pRenderer, double angle, int alpha, bool centered, SDL_RendererFlip flip)
{
	SDL_Rect srcRect;
	SDL_Rect destRect;
	...
	//std::cout << "Texture W: " << textureWidth << std::endl;

	srcRect.w = destRect.w = (textureWidth/SpriteMax);
	srcRect.h = destRect.h = textureHeight;

	//std::cout << "Src W: " << srcRect.w << std::endl;

	if (centered) {
		int xOffset = (textureWidth/6) * 0.5;
		int yOffset = textureHeight * 0.5;
		destRect.x = x - xOffset;
		destRect.y = y - yOffset;
	}
